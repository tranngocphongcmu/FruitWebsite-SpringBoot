	...
	<div id="content-wrapper" class="d-flex flex-column">
		...
		<div id="content">
			...
			<div class="d-sm-flex align-items-center justify-content-between mb-4">
				<div class="container">
					<h2>User Table</h2>
					<td><a href="/addUser"><button class="btn btn-primary"  type="button" data-dismiss="modal">Add</button></a></td>
					...
					<table class="table table-bordered">
						...
						<tbody>
						<c:forEach var="item" items="${useradmin.content}">
							<tr>
								<td>${item.id}</td>
								<td>${item.name}</td>
								<td>${item.email}</td>
								...
								<td>${item.address}</td>
								<td>${item.password}</td>
								<td><a href="/updateUser/${item.id}"><button class="btn btn-primary" type="button" data-dismiss="modal">Update</button></a></td>
								<td><a href="/deleteuser/${item.id}"><button class="btn btn-primary" type="button" data-dismiss="modal">Delete</button></a></td>
								<td><a href="/get-single-user/${item.id}"><button class="btn btn-primary" type="button" data-dismiss="modal">View</button></a></td>
							</tr>
						</c:forEach>
						</tbody>
					</table>
		...
		<div class="container my-auto">
			<div class="center">
				...
				<div class="pagination">
					...
					<c:if test="${useradmin.totalPages > 1}">
						<ul>
							<c:forEach var="i" begin="1" end="${useradmin.totalPages}">
